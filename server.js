...
app.get('/', (req, res) => {
    res.send(
        ...
        'body: JSON.stringify({ bot_username: username, autotrade: autotrade, item_target: itemTargetString, receiver: receiver })' +
        '});' +
        'closeTradeModal();' +
        'refreshData();' +
        '}' +
        ...
        'return;' +
        '}' +
        'const filtered = bot.groupedInventory.filter(item => item.name.toLowerCase().includes(filterKeyword.toLowerCase()));' +
        'if (filtered.length === 0) {' +
